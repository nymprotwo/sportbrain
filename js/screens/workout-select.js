...
Screens.workoutSelect = function() {
        ...
        if (effectiveCatId) {
          ...
          if (cat) {
            ...
            if (cat.types.length > 1) {
              ...
              html += '<div style="display:flex;flex-wrap:wrap;gap:7px">';
              cat.types.forEach(t => {
                html += chip(t.name, effectiveTypeId === t.id, 'Diary.selectorSetType(\'' + t.id + '\')');
              });
              html += '</div></div>';
            }
              ...
              if (type) {
                ...
                const _renderEquips = () => {
                  ...
                  effEquips.forEach(eqId => {
                    ...
                    if (icon) {
                      const shortNames = {cable_wide:'Широкая',cable_v:'V-рукоять',cable_single:'Одиночный',cable_d:'D-рукоять',cable_rope:'Канат',cable_narrow:'Узкая'};
                      const label = (type.equipNames && type.equipNames[eqId]) || shortNames[eqId] || eq.name;
                      html += '<button onclick="' + fn + '" style="display:flex;flex-direction:column;align-items:center;gap:5px;padding:10px 12px;border-radius:12px;border:2px solid ' + (active ? 'var(--ac)' : '#2a2a2a') + ';background:' + (active ? 'rgba(160,120,48,0.15)' : '#1a1a1a') + ';cursor:pointer;min-width:72px;font-family:inherit">' +
                        '<img src="' + icon + '" style="width:40px;height:40px;object-fit:contain;border-radius:6px" />' +
                        '<span style="font-size:10px;color:' + (active ? 'var(--ac)' : 'var(--tx)') + ';font-weight:' + (active ? '700' : '400') + ';white-space:nowrap">' + label + '</span>' +
                        '</button>';
                    } else {
                      html += chip(eq.name, active, fn);
                    }
                  });
                  html += '</div></div>';
                };
    ...
    if (plan.length > 0) {
      ...
      plan.forEach((p, i) => {
        const displayName = formatExName(p.name, p.equip, p.accent, p.muscle);
