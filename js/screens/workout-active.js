...
Screens.workoutActive = function() {
    ...
    if (d.activeIdx === -1) {
      ...
      html += '<button class="btn btn-ghost" onclick="Storage.saveTraining();State.diary.stage=\'browse\';State.diary.activeIdx=-1;App.render()">‹</button>';
      html += '<div><span class="lbl">ТРЕНИРОВКА</span>';
      html += '<div class="ttl" style="font-size:28px">Текущая тренировка</div></div>';
      html += '</div>';

      // ── Live session stats ────────────────────────────────
      const _allWS   = d.workout.flatMap(ex => ex.sets.filter(s => s.type === 'work'));
      const _sesTon  = _allWS.reduce((a, s) =>
        a + (s.weightMode === 'bw' || s.weightMode === 'assist' ? 0 : (s.weight||0) * (s.reps||0)), 0);
      const _sesSets = _allWS.length;
      const _sesExes = d.workout.filter(ex => ex.sets.some(s => s.type === 'work')).length;

      // Last workout & PB
      const _lastW   = DB.workouts[0];
      const _lastTon = _lastW ? _lastW.exercises.reduce((a, ex) =>
        a + ex.sets.filter(s => s.type === 'work').reduce((b, s) => b + s.weight * s.reps, 0), 0) : 0;
      const _pb      = DB.workouts.reduce((best, w) => {
        const t = w.exercises.reduce((a, ex) =>
          a + ex.sets.filter(s => s.type === 'work').reduce((b, s) => b + s.weight * s.reps, 0), 0);
        return t > best ? t : best;
      }, 0);

      // Intensity (avg reps of work sets)
      const _avgReps = _allWS.length > 0 ? _allWS.reduce((a, s) => a + (s.reps||0), 0) / _allWS.length : 0;
      const _intens  = _avgReps === 0 ? null : _avgReps <= 5 ? 'высокая' : _avgReps <= 10 ? 'средняя' : 'низкая';
      const _inCol   = _intens === 'высокая' ? 'var(--re)' : _intens === 'средняя' ? 'var(--ac)' : 'var(--sb)';

      // Comparison flags
      const _isRec  = _sesTon > 0 && _pb > 0 && _sesTon > _pb;
      const _isBett = _sesTon > 0 && !_isRec && _lastTon > 0 && _sesTon > _lastTon;
      const _diff   = _sesTon - _lastTon;
      const _tonFmt = t => t >= 1000 ? (t/1000).toFixed(1) + ' т' : t + ' кг';

      // Card border reacts to state
      const _cardBdr = _isRec ? 'var(--ac)' : _isBett ? 'rgba(74,222,128,.35)' : 'var(--br)';
      const _cardGlw = _isRec ? ';box-shadow:0 0 24px var(--ac15)' : '';

      html += '<div style="background:var(--sf);border:1px solid ' + _cardBdr + ';border-radius:14px;padding:14px 16px;margin-bottom:20px' + _cardGlw + '">';

      // Top row: label + progress counter
      html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">';
      html += '<div style="font-size:9px;color:var(--sb);letter-spacing:3px">ТЕКУЩАЯ СЕССИЯ</div>';
      const _progCol = doneCount === totalCount && totalCount > 0 ? 'var(--color-success)' : 'var(--ac)';
      html += '<div style="font-size:12px;font-weight:700;color:' + _progCol + '">' + doneCount + ' / ' + totalCount + '</div>';
      html += '</div>';

      if (_sesTon === 0) {
        // No sets yet — simple hint
        html += '<div style="font-size:12px;color:var(--sb)">👇 Нажми на упражнение, чтобы начать</div>';
      } else {
        // ── Тоннаж + интенсивность ──────────
        html += '<div style="display:flex;align-items:baseline;gap:10px;margin-bottom:6px">';
        html += '<div style="font-size:30px;font-weight:700;color:var(--ac);line-height:1">' + _tonFmt(_sesTon) + '</div>';
        if (_intens) {
          html += '<div style="font-size:10px;font-weight:700;color:' + _inCol + ';letter-spacing:.3px">' + _intens + '</div>';
        }
        html += '</div>';

        // ── Мета ────────────────────────────
        html += '<div style="font-size:10px;color:var(--sb);margin-bottom:10px">' +
          _sesSets + ' ' + (_sesSets === 1 ? 'подход' : _sesSets < 5 ? 'подхода' : 'подходов') +
          ' &nbsp;·&nbsp; ' + _sesExes + ' ' + (_sesExes === 1 ? 'упражнение' : _sesExes < 5 ? 'упражнения' : 'упражнений') +
          '</div>';
        ...
        if (_lastTon > 0 || _pb > 0) {
          ...
          if (_lastTon > 0) {
            const _dStr  = _diff >= 0 ? '+' + _tonFmt(_diff) : '−' + _tonFmt(-_diff);
            const _dCol  = _diff >= 0 ? 'var(--color-success)' : 'var(--sb)';
            html += '<div style="font-size:10px;color:var(--sb)">Прошлая&nbsp;<span style="color:var(--tx)">' + _tonFmt(_lastTon) + '</span>&nbsp;<span style="color:' + _dCol + ';font-weight:700">' + _dStr + '</span></div>';
          }
          if (_pb > 0) {
            html += '<div style="font-size:10px;color:var(--sb)">Рекорд&nbsp;<span style="color:var(--tx)">' + _tonFmt(_pb) + '</span>' + (_isRec ? '&nbsp;<span style="color:var(--ac)">★</span>' : '') + '</div>';
          }
          html += '</div>';
        }
        ...
        if (_isRec) {
          html += '<div style="margin-top:10px;padding:5px 12px;border-radius:8px;background:var(--ac15);border:1px solid var(--ac30);display:inline-flex;align-items:center;gap:5px">';
          html += '<span style="font-size:11px;font-weight:700;color:var(--ac)">🔥 Новый рекорд сессии!</span>';
          html += '</div>';
        } else if (_isBett) {
          const _bStr = _diff >= 1000 ? '+' + (_diff/1000).toFixed(1) + ' т' : '+' + _diff + ' кг';
          html += '<div style="margin-top:10px;padding:5px 12px;border-radius:8px;background:rgba(74,222,128,.07);border:1px solid rgba(74,222,128,.2);display:inline-flex;align-items:center;gap:5px">';
          html += '<span style="font-size:11px;color:var(--color-success)">↑ ' + _bStr + ' к прошлой тренировке</span>';
          html += '</div>';
        }
      }
      ...
      d.workout.forEach((w, i) => {
        ...
        const displayName = formatExName(w.name, w.equip, w.accent, w.muscle, w.id);
        ...
        if (isDone && workSets > 0) {
          ...
          html += '<span style="padding:2px 7px;border-radius:6px;border:1px solid rgba(80,140,80,.2);background:rgba(80,140,80,.08);font-size:9px;color:#6a8a6a">' + workSets + ' раб. подх</span>';
          html += '</div>';
        }
        html += '</div>';
        html += statusEl;
      ...
      const totalWorkSets = d.workout.reduce((s, w) => s + w.sets.filter(st => st.type === 'work').length, 0);
      ...
      if (totalWorkSets === 0) {
        // Ни одного рабочего подхода — нельзя завершать
        html += '<button disabled style="width:100%;padding:13px;border-radius:14px;border:1px solid #1a1b1e;background:transparent;color:#303238;font-size:13px;font-weight:600;font-family:inherit;cursor:not-allowed;letter-spacing:.5px;opacity:.5">✓ Завершить тренировку</button>';
        html += '<div style="text-align:center;font-size:10px;color:#404248;margin-top:-4px;letter-spacing:.3px">Тренировка ещё не начата</div>';
      ...
      // Выйти без потери прогресса
      html += '<button onclick="Storage.saveTraining();State.diary.stage=\'browse\';State.diary.activeIdx=-1;App.render()" style="width:100%;padding:10px;border-radius:12px;border:1px solid #1e1f22;background:transparent;color:#555760;font-size:11px;font-family:inherit;cursor:pointer;letter-spacing:.3px">↙ Выйти и продолжить позже</button>';

      // Отменить (деструктивно, малозаметно)
      html += '<button onclick="if(confirm(\'Отменить тренировку? Весь прогресс будет удалён.\')){Storage.clearCurrent();State.diary.workout=[];State.diary.activeIdx=-1;State.diary.stage=\'browse\';App.render()}" style="background:none;border:none;color:#2a2c30;font-size:10px;cursor:pointer;font-family:inherit;padding:2px;letter-spacing:.3px">Отменить тренировку</button>';
    ...
    // Постановка ног — только приседания; для них grip не показывается
    const showStance    = STANCE_EX_IDS.has(ex.id);
    const stanceObj     = STANCE_LIST.find(s => s.id === (ex.stance || 'shoulder'));
    // Хват (тип + ширина) только для штанги и EZ-грифа, но НЕ для приседаний и жимов
    const showGripType  = GRIP_TYPE_EQUIPS.has(ex.equip) && !showStance && !PRESS_EX_IDS.has(ex.id);
    const showGripWidth = GRIP_WIDTH_EQUIPS.has(ex.equip) && !showStance;
    // Вариации — фильтруем по текущему снаряду
    const exVariations  = (EX_VARIATIONS[ex.id] || []).filter(v => !v.equip || !v.equip.length || v.equip.includes(ex.equip));
    const variationObj  = ex.variation ? exVariations.find(v => v.id === ex.variation) : null;
    const gripTypeObj   = GRIP_TYPES.find(g => g.id === ex.gripType);
    const gripWidthObj  = GRIP_WIDTHS.find(g => g.id === ex.gripWidth);
    ...
    // Config key: движение_снаряд_[постановка|хват_тип_ширина]_акцент_темп
    const configKey = [
      ex.id, ex.equip,
      showStance  ? (ex.stance || 'shoulder') : null,
      !showStance ? ex.gripType  : null,
      !showStance ? ex.gripWidth : null,
      ex.accent, ex.tut !== 'normal' ? ex.tut : null,
    ...
    if (stanceObj && showStance)       semParts.push(stanceObj.name);
    if (gripTypeObj && showGripType)   semParts.push(gripTypeObj.name + ' хват');
    if (gripWidthObj && showGripWidth) semParts.push(gripWidthObj.name);
    ...
    const tutChips = (activeId, allowedIds) => TUT_LIST.filter(item => !allowedIds || allowedIds.includes(item.id)).map(item => {
      const on = activeId === item.id;
    ...
    const paramsBlock = () => {
      ...
      if (State.settings.tempo) {
        const tutOpts = TUT_EX_OPTIONS[ex.id] !== undefined ? TUT_EX_OPTIONS[ex.id] : ['pause1','negative3'];
        if (tutOpts.length > 0) {
          const tutOn = !!ex.tutExpanded;
          h += '<div style="margin-bottom:12px">';
          h += '<div style="display:flex;align-items:center;justify-content:space-between' + (tutOn ? ';margin-bottom:6px' : '') + '">';
          h += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px">ТЕМП</div>';
          h += '<div onclick="Diary.updateExField(\'tutExpanded\',' + !tutOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (tutOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
          h += '<div style="width:16px;height:16px;border-radius:50%;background:white;position:absolute;top:2px;left:' + (tutOn ? '18px' : '2px') + '"></div>';
          h += '</div></div>';
          if (tutOn) {
            h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + tutChips(ex.tut, tutOpts) + '</div>';
          }
          h += '</div>';
        }
      }
      ...
      // Время отдыха
      {
        const restOn = !!ex.restExpanded;
        h += '<div style="margin-bottom:12px">';
        h += '<div style="display:flex;align-items:center;justify-content:space-between' + (restOn ? ';margin-bottom:6px' : '') + '">';
        h += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px">ВРЕМЯ ОТДЫХА</div>';
        h += '<div onclick="Diary.updateExField(\'restExpanded\',' + !restOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (restOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
        h += '<div style="width:16px;height:16px;border-radius:50%;background:white;position:absolute;top:2px;left:' + (restOn ? '18px' : '2px') + '"></div>';
        h += '</div></div>';
        if (restOn) {
          h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(REST_LIST, ex.rest || '120', 'rest') + '</div>';
        }
        h += '</div>';
      }
      ...
      if (showStance) {
        h += '<div style="margin-bottom:12px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ПОСТАНОВКА НОГ</div>';
        h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(STANCE_LIST, ex.stance || 'shoulder', 'stance') + '</div></div>';
      }
      if (PRESS_EX_IDS.has(ex.id)) {
        if (showGripWidth) {
          h += '<div style="margin-bottom:12px">';
          h += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ШИРИНА ХВАТА</div>';
          h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_WIDTHS, ex.gripWidth, 'gripWidth') + '</div>';
          {
            const hasVar = exVariations.length > 0;
            const varOn  = !!ex.variationExpanded && hasVar;
            h += '<div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding-top:10px;border-top:1px solid #1a1c20' + (varOn ? ';margin-bottom:8px' : '') + ';opacity:' + (hasVar ? '1' : '0.3') + '">';
            h += '<div style="font-size:8px;color:#3a3c42;letter-spacing:1.5px">ВАРИАЦИИ</div>';
            if (hasVar) {
              h += '<div onclick="Diary.updateExField(\'variationExpanded\',' + !varOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (varOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
            } else {
              h += '<div style="width:36px;height:20px;border-radius:10px;background:#222;position:relative;flex-shrink:0">';
            }
            h += '<div style="width:16px;height:16px;border-radius:50%;background:' + (hasVar ? 'white' : '#444') + ';position:absolute;top:2px;left:' + (varOn ? '18px' : '2px') + '"></div>';
            h += '</div></div>';
            if (varOn) {
              h += '<div style="display:flex;flex-wrap:wrap;gap:6px">';
              exVariations.forEach(v => {
                const on = ex.variation === v.id;
                h += '<button onclick="Diary.setVariation(\'' + v.id + '\')" style="padding:6px 12px;border-radius:16px;border:1px solid ' + (on?'var(--ac)':'var(--br)') + ';background:' + (on?'var(--ad)':'var(--sf)') + ';color:' + (on?'var(--ac)':'var(--mt)') + ';font-size:11px;cursor:pointer;font-family:inherit;font-weight:' + (on?'700':'400') + ';transition:all .15s">' + v.name + '</button>';
              });
              h += '</div>';
            }
          }
        ...
        if (showGripType) {
          h += '<div style="margin-bottom:12px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ТИП ХВАТА</div>';
          h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_TYPES, ex.gripType, 'gripType') + '</div></div>';
        }
        if (showGripWidth) {
          h += '<div style="margin-bottom:12px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ШИРИНА ХВАТА</div>';
          h += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_WIDTHS, ex.gripWidth, 'gripWidth') + '</div></div>';
        }
      }
      return h;
    };
    ...
    if (!ex.setupDone) {
      ...
      // ── Таблица эффектов ──────────────────────
      // Хват: per-exercise lookup из data.js, _default — запасной
      const gripTypeFxMap  = EX_GRIP_TYPE_FX[ex.id]  || EX_GRIP_TYPE_FX._default;
      const gripWidthFxMap = EX_GRIP_WIDTH_FX[ex.id] || EX_GRIP_WIDTH_FX._default;
      const TUT_EFFECTS = {
        normal:    'Натуральный темп · развитие максимальной силы',
        pause1:    'Пауза 1 с · устраняет инерцию · стартовая сила',
        pause3:    'Пауза 3 с · максимальный контроль · сила из статики',
        pause_mid: 'Пауза в слабой точке · контроль траектории · стабилизация',
        negative3: 'Медленный негатив · время под нагрузкой · гипертрофия',
        slow:      'Темп 4-0-4 · максимальное напряжение во всей амплитуде',
      };
      const REST_EFFECTS = {
      ...
      const stanceEffect    = showStance    ? (STANCE_EFFECTS[ex.stance || 'shoulder'] || null) : null;
      const gripTypeEffect  = showGripType  ? (gripTypeFxMap[ex.gripType]   || null) : null;
      const gripWidthEffect = showGripWidth ? (gripWidthFxMap[ex.gripWidth]  || null) : null;
      const tutEffect  = (ex.tutExpanded && ex.tut !== 'normal') ? (TUT_EFFECTS[ex.tut] || null) : null;
      const restEffect = (ex.restExpanded && ex.rest !== '120') ? (REST_EFFECTS[ex.rest] || null) : null;
      const hasEffect  = stanceEffect || gripTypeEffect || gripWidthEffect || tutEffect || restEffect;
      ...
      // 1. Постановка ног — только приседания
      if (showStance) {
        html += '<div style="margin-bottom:14px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ПОСТАНОВКА НОГ</div>';
        html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(STANCE_LIST, ex.stance || 'shoulder', 'stance') + '</div></div>';
      }

      // 1а/б. Хват + вариации
      if (PRESS_EX_IDS.has(ex.id)) {
        // Жим: ШИРИНА ХВАТА (всегда) + ВАРИАЦИИ как toggle (если есть для текущего снаряда)
        if (showGripWidth) {
          html += '<div style="margin-bottom:14px">';
          html += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ШИРИНА ХВАТА</div>';
          html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_WIDTHS, ex.gripWidth, 'gripWidth') + '</div>';
          {
            const hasVar = exVariations.length > 0;
            const varOn  = !!ex.variationExpanded && hasVar;
            html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding-top:10px;border-top:1px solid #1a1c20' + (varOn ? ';margin-bottom:8px' : '') + ';opacity:' + (hasVar ? '1' : '0.3') + '">';
            html += '<div style="font-size:8px;color:#3a3c42;letter-spacing:1.5px">ВАРИАЦИИ</div>';
            if (hasVar) {
              html += '<div onclick="Diary.updateExField(\'variationExpanded\',' + !varOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (varOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
            } else {
              html += '<div style="width:36px;height:20px;border-radius:10px;background:#222;position:relative;flex-shrink:0">';
            }
            html += '<div style="width:16px;height:16px;border-radius:50%;background:' + (hasVar ? 'white' : '#444') + ';position:absolute;top:2px;left:' + (varOn ? '18px' : '2px') + '"></div>';
            html += '</div></div>';
            if (varOn) {
              html += '<div style="display:flex;flex-wrap:wrap;gap:6px">';
              exVariations.forEach(v => {
                const on = ex.variation === v.id;
                html += '<button onclick="Diary.setVariation(\'' + v.id + '\')" style="padding:6px 12px;border-radius:16px;border:1px solid ' + (on?'var(--ac)':'var(--br)') + ';background:' + (on?'var(--ad)':'var(--sf)') + ';color:' + (on?'var(--ac)':'var(--mt)') + ';font-size:11px;cursor:pointer;font-family:inherit;font-weight:' + (on?'700':'400') + ';transition:all .15s">' + v.name + '</button>';
              });
              html += '</div>';
            }
          }
        ...
        // Тяги и прочие: тип хвата первым, потом ширина
        if (showGripType) {
          html += '<div style="margin-bottom:14px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ТИП ХВАТА</div>';
          html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_TYPES, ex.gripType, 'gripType') + '</div></div>';
        }
        if (showGripWidth) {
          html += '<div style="margin-bottom:14px"><div style="font-size:9px;color:var(--sb);letter-spacing:2px;margin-bottom:6px">ШИРИНА ХВАТА</div>';
          html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(GRIP_WIDTHS, ex.gripWidth, 'gripWidth') + '</div></div>';
        }
      }
      ...
      if (State.settings.tempo) {
        ...
        if (tutOpts.length > 0) {
          const tutOn = !!ex.tutExpanded;
          html += '<div style="margin-bottom:14px">';
          html += '<div style="display:flex;align-items:center;justify-content:space-between' + (tutOn ? ';margin-bottom:8px' : '') + '">';
          html += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px">ТЕМП</div>';
          html += '<div onclick="Diary.updateExField(\'tutExpanded\',' + !tutOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (tutOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
          html += '<div style="width:16px;height:16px;border-radius:50%;background:white;position:absolute;top:2px;left:' + (tutOn ? '18px' : '2px') + '"></div>';
          html += '</div></div>';
          if (tutOn) {
            html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + tutChips(ex.tut, tutOpts) + '</div>';
          }
          html += '</div>';
        }
      }
      ...
      // 4. Время отдыха
      {
        const restOn = !!ex.restExpanded;
        html += '<div>';
        html += '<div style="display:flex;align-items:center;justify-content:space-between' + (restOn ? ';margin-bottom:8px' : '') + '">';
        html += '<div style="font-size:9px;color:var(--sb);letter-spacing:2px">ВРЕМЯ ОТДЫХА</div>';
        html += '<div onclick="Diary.updateExField(\'restExpanded\',' + !restOn + ')" style="width:36px;height:20px;border-radius:10px;background:' + (restOn ? 'var(--ac)' : '#333') + ';position:relative;cursor:pointer;flex-shrink:0">';
        html += '<div style="width:16px;height:16px;border-radius:50%;background:white;position:absolute;top:2px;left:' + (restOn ? '18px' : '2px') + '"></div>';
        html += '</div></div>';
        if (restOn) {
          html += '<div style="display:flex;flex-wrap:wrap;gap:6px">' + chips(REST_LIST, ex.rest || '120', 'rest') + '</div>';
        }
        html += '</div>';
      }
      ...
      if (hasEffect) {
        ...
        if (stanceEffect) {
          html += '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:' + (gripTypeEffect || gripWidthEffect || tutEffect || restEffect ? '6px' : '0') + '">';
          html += '<span style="font-size:9px;color:#3a5a3a;letter-spacing:1px;white-space:nowrap">ПОСТАНОВКА</span>';
          html += '<span style="font-size:12px;color:#7aaa7a">→ ' + stanceEffect + '</span></div>';
        }
        if (gripTypeEffect) {
          html += '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:' + (gripWidthEffect || tutEffect || restEffect ? '6px' : '0') + '">';
          html += '<span style="font-size:9px;color:#3a5a3a;letter-spacing:1px;white-space:nowrap">ХВАТ</span>';
          html += '<span style="font-size:12px;color:#7aaa7a">→ ' + gripTypeEffect + '</span></div>';
        }
        if (gripWidthEffect) {
          html += '<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:' + (tutEffect || restEffect ? '6px' : '0') + '">';
          html += '<span style="font-size:9px;color:#3a5a3a;letter-spacing:1px;white-space:nowrap">ШИРИНА</span>';
          html += '<span style="font-size:12px;color:#7aaa7a">→ ' + gripWidthEffect + '</span></div>';
        }
        if (tutEffect) {
        ...
        if (restEffect) {
          ...
          html += '<span style="font-size:9px;color:#2a4a2a;letter-spacing:1px;white-space:nowrap">ВРЕМЯ ОТДЫХА</span>';
    ...
    [equipObj ? equipObj.name : null, accentObj ? accentObj.name : null, (stanceObj && showStance) ? stanceObj.name : null, (gripTypeObj && showGripType) ? gripTypeObj.name + ' хват' : null, (gripWidthObj && showGripWidth) ? gripWidthObj.name : null, variationObj ? variationObj.name : null, ex.tut !== 'normal' ? tutShort : null, maxRpe ? 'RPE ' + maxRpe : null]
    ...
    // ── Заметка (если есть) ───────────────────────
    if (ex.note) {
      html += '<div onclick="State.modal={type:\'note\'};App.renderModal()" style="background:#1a1a1f;border:1px solid #2e2e38;border-left:3px solid #e8c84a;border-radius:10px;padding:9px 12px;margin-bottom:10px;cursor:pointer;display:flex;align-items:flex-start;gap:8px">';
      html += '<span style="font-size:14px;flex-shrink:0;margin-top:1px">📝</span>';
      html += '<div style="flex:1;min-width:0">';
      html += '<div style="font-size:9px;color:#e8c84a;letter-spacing:1.5px;margin-bottom:3px">ЗАМЕТКА</div>';
      html += '<div style="font-size:12px;color:#C8CACC;line-height:1.5;word-break:break-word">' + ex.note.replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</div>';
      html += '</div></div>';
    }
    ...
    const simple   = State.settings.simpleMode;
    ...
    if (maxRows === 0) {
      ...
      for (let ri = 0; ri < maxRows; ri++) {
        ...
        if (hs) {
          ...
          const hwDisp = hs.weightMode === 'bw' ? 'СВ' : (hs.weightMode === 'assist' ? '−' + dispW(hs.weight) + unitLbl() : dispW(hs.weight) + (isLb ? 'lb' : ''));
        ...
        if (cs) {
          if (!simple) html += '<span style="font-size:9px;font-weight:700;color:' + cc.col + ';width:22px;flex-shrink:0">' + cc.label + '</span>';
          const cwDisp = cs.weightMode === 'bw' ? 'СВ' : (cs.weightMode === 'assist' ? '−' + dispW(cs.weight) + unitLbl() : dispW(cs.weight) + (isLb ? 'lb' : ''));
          html += '<span style="font-size:13px;color:#EAEAEA;font-weight:400">' + cwDisp + '×' + cs.reps + '</span>';
          if (cs.rpe && !simple) html += '<span style="font-size:10px;color:' + RPE_COL + ';font-weight:600;margin-left:4px">RPE ' + cs.rpe + '</span>';
        ...
        if (isEditing && cs) {
          ...
          if (csShowW) {
            const csDw   = dispW(cs.weight);
            const csDecKg = Math.max(0, cs.weight - unitStep());
            const csIncKg = cs.weight + unitStep();
            const csWBase = WEIGHT_MODE_EX.includes(ex.id) ? (csWm === 'assist' ? 'АССИСТ' : '+ВЕС') : 'ВЕС';
            html += '<div style="min-width:0">';
            html += '<div style="font-size:8px;color:#A0A2A8;letter-spacing:1px;margin-bottom:4px">' + csWBase + ', ' + unitLbl() + '</div>';
            html += '<div style="display:flex;align-items:center;gap:3px;overflow:hidden">';
            html += '<button onclick="Diary.updateSetField(' + ri + ',\'weight\',' + csDecKg + ')" style="' + rBtnSt(cc.col) + '">−</button>';
            html += '<input type="text" inputmode="decimal" value="' + csDw + '" onfocus="this.select()" onblur="Diary.updateSetField(' + ri + ',\'weight\',inputToKg(parseFloat(this.value.replace(\',\',\'.\'))||0))" style="' + rInpSt + '">';
            html += '<button onclick="Diary.updateSetField(' + ri + ',\'weight\',' + csIncKg + ')" style="' + rBtnSt(cc.col) + '">+</button>';
            html += '</div></div>';
          }
    ...
    if (histAllTon > 0 || curAllTon > 0) {
      ...
      if (histAllTon > 0) html += '<span style="font-size:12px;font-weight:400;color:#7E8088">' + (isLb ? kgToLb(histAllTon) + ' lb' : histAllTon + ' кг') + '</span>';
      html += '</div>';
      ...
      html += '</div>';
      html += '<div style="text-align:center">';
      if (curAllTon > 0) html += '<span style="font-size:12px;font-weight:500;color:var(--ac)">' + (isLb ? kgToLb(curAllTon) + ' lb' : curAllTon + ' кг') + '</span>';
      html += '</div>';
      html += '</div>';
    }
    ...
    // ── Тумблер простого режима ──────────────────
    html += '<div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-bottom:6px;padding:0 2px">';
    html += '<span style="font-size:9px;color:' + (simple ? 'var(--ac)' : '#454750') + ';letter-spacing:.5px;transition:color .2s">' + (simple ? 'ТОЛЬКО ВЕС · ПОВТОРЫ' : 'только вес · повторы') + '</span>';
    html += '<div onclick="toggleSimpleMode()" style="width:32px;height:18px;border-radius:9px;background:' + (simple ? 'var(--ac)' : '#252528') + ';position:relative;cursor:pointer;flex-shrink:0;border:1px solid ' + (simple ? 'var(--ac)' : '#333') + ';transition:background .2s">';
    html += '<div style="width:14px;height:14px;border-radius:50%;background:' + (simple ? '#fff' : '#666') + ';position:absolute;top:1px;left:' + (simple ? '15px' : '1px') + ';transition:left .15s"></div>';
    html += '</div></div>';
    ...
    // ══ Форма добавления подхода ══════════════════
    {
      ...
      // Тип: 3 компактные пилюли (скрываем в простом режиме)
      if (!simple) {
        html += '<div style="display:flex;gap:5px;margin-bottom:8px">';
        ['warmup','leadup','work'].forEach(t => {
          const tc = SET_COL[t];
      ...
      if (showWeight) {
        const inpDw  = dispW(inp.weight);
        const decKg  = Math.max(0, inp.weight - unitStep());
        const incKg  = inp.weight + unitStep();
        const wBase  = WEIGHT_MODE_EX.includes(ex.id) ? (wm === 'assist' ? 'АССИСТ' : '+ВЕС') : 'ВЕС';
        html += '<div style="min-width:0">';
        // Label + маленький kg/lb переключатель
        html += '<div style="font-size:9px;color:#B4B6BC;letter-spacing:1px;margin-bottom:3px;display:flex;align-items:center;gap:5px">';
        html += wBase + ' ';
        html += '<span onclick="saveUnit(\'kg\');App.render()" style="cursor:pointer;padding:1px 6px;border-radius:4px;font-size:8px;' +
          'border:1px solid ' + (!isLb ? 'var(--ac)' : '#2a2a30') + ';' +
          'color:' + (!isLb ? 'var(--ac)' : '#555') + ';' +
          'background:' + (!isLb ? 'var(--ac12)' : 'transparent') + '">кг</span>';
        html += '<span onclick="saveUnit(\'lb\');App.render()" style="cursor:pointer;padding:1px 6px;border-radius:4px;font-size:8px;' +
          'border:1px solid ' + (isLb ? 'var(--ac)' : '#2a2a30') + ';' +
          'color:' + (isLb ? 'var(--ac)' : '#555') + ';' +
          'background:' + (isLb ? 'var(--ac12)' : 'transparent') + '">lb</span>';
        html += '</div>';
        html += '<div style="display:flex;align-items:center;gap:4px;overflow:hidden">';
        html += '<button onclick="Diary.updateInput(\'weight\',' + decKg + ')" style="' + iBtnSt(ic.col) + '">−</button>';
        html += '<input type="text" inputmode="decimal" value="' + inpDw + '" onfocus="this.select()" onblur="Diary.updateInput(\'weight\',inputToKg(parseFloat(this.value.replace(\',\',\'.\'))||0))" style="' + iInpSt + '">';
        html += '<button onclick="Diary.updateInput(\'weight\',' + incKg + ')" style="' + iBtnSt(ic.col) + '">+</button>';
        html += '</div></div>';
      }
      ...
      // RPE [6][7][8][9][10] — только для рабочих и не в простом режиме
      if (inp.type === 'work' && !simple) {
...
Screens.workoutRegressCheck = function() {
    ...
    html += '<button class="btn btn-ghost" style="margin-bottom:16px" onclick="State.diary.stage=\'active\';App.render()">‹ Вернуться к тренировке</button>';
