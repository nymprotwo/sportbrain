...
Screens.programDetail = function() {
    ...
    if (isActive) {
      ...
      html += '<button onclick="if(confirm(\'Сбросить прогресс программы?\')){clearActiveProgram();Storage.clearCurrent();State.diary.workout=[];State.diary.activeIdx=-1;State.diary.stage=\'browse\';App.render()}" style="background:none;border:none;color:rgba(192,57,43,.6);font-size:10px;cursor:pointer;font-family:inherit">сбросить</button>';
      html += '</div>';
      html += '</div>';
    } else {
...
Screens.programs = function() {
    ...
    const backTab = State.prevTab || 'workout';
    let html = '';
    html += '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">';
    html += '<button class="btn btn-ghost" onclick="App.nav(\'' + backTab + '\')">‹</button>';
    html += '<div><span class="lbl">БАЗА ЗНАНИЙ</span>';
    html += '<div class="ttl" style="font-size:28px;line-height:1.1">Программы</div></div>';
    html += '</div>';
