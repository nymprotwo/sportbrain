...
const App = {
  nav(tab) {
    // Запоминаем откуда пришли (для кнопки «назад» в Programs)
    if (tab === 'programs' && State.tab !== 'programs') State.prevTab = State.tab;
    State.tab = tab;
    // При переключении вкладки сбрасываем страницу настроек
    if (tab !== 'settings') State.settingsPage = null;
    // При нажатии на вкладку «Тренировка» — всегда возвращаем на главный экран раздела.
    // Активная тренировка остаётся в State.diary.workout, кнопка «Продолжить» появится на главном.
    if (tab === 'workout') {
      State.orm.active = false;
      State.diary.historyDetailId = null;
  ...
  // Универсальный «шаг назад» — используется кнопками ‹ на всех экранах
  goBack() {
    // Programs
    if (State.tab === 'programs') {
      if (State.programNav.active) {
        State.programNav.active = false; State.programNav.weekIdx = null; State.programNav.dayIdx = null;
        this.render();
      } else {
        this.nav(State.prevTab || 'workout');
      }
      return;
    }
    // ORM
    if (State.orm.active) {
      if (State.orm.step > 0) State.orm.step = State.orm.step === 3 ? 1 : State.orm.step - 1;
      else State.orm.active = false;
      this.render(); return;
    }
    // Workout sub-screens
    if (State.tab === 'workout') {
      const s = State.diary.stage;
      if (s === 'regress_check')                      { State.diary.stage = 'active'; this.render(); return; }
      if (s === 'history' && State.diary.historyDetailId) { State.diary.historyDetailId = null; this.render(); return; }
      if (s === 'history')                            { State.diary.stage = 'browse'; this.render(); return; }
      if (s === 'select')                             { const mid = State.diary.midWorkoutAdd; State.diary.midWorkoutAdd = false; State.diary.stage = mid ? 'active' : 'browse'; this.render(); return; }
      if (s === 'plan')                               { State.diary.stage = 'select'; this.render(); return; }
      if (s === 'build')                              { State.diary.stage = 'browse'; this.render(); return; }
      if (s === 'active' && State.diary.activeIdx !== -1) { State.diary.activeIdx = -1; this.render(); return; }
    }
  },
  ...
  goToHistory() {
    ...
    State.orm.active = false;
    State.diary.historyDetailId = null;
    State.diary.stage = 'history';
    document.querySelectorAll('.nav button').forEach(b => {
      b.classList.toggle('active', b.id === 'nav-workout');
    });
    this.render();
  },
  ...
  startORM() {
    State.tab = 'workout';
    State.diary.stage = 'browse';
    State.orm.active = true;
    State.orm.step   = 0;
    State.orm.method   = null;
    State.orm.exercise = null;
    State.orm.result   = null;
    document.querySelectorAll('.nav button').forEach(b => {
      b.classList.toggle('active', b.id === 'nav-workout');
    });
    this.render();
  ...
  render() {
    ...
    try {
      ...
      if (State.tab === 'settings') sc.innerHTML = Screens.settings();
    } catch(e) {
  ...
  renderModal() {
    ...
    const fn = {
      swap: Modals.swap, note: Modals.note, program: Modals.program,
      dark: Modals.dark, confirmClear: Modals.confirmClear, confirmDelete: Modals.confirmDelete,
      goalAdd: Modals.goalAdd, achDetail: Modals.achDetail
    };
...
const Diary = {
    ...
    State.diary.muscles.forEach(muscle => {
      (State.diary.selections[muscle] || []).forEach(exId => {
        ...
        if (ex) blocks.push({
          ...
          gripType: 'straight', gripWidth: 'medium', stance: 'shoulder',
          equip: (ex.equip && ex.equip[0]) || 'barbell', tut: 'normal', rest: '120',
          accent: (ex.accents && ex.accents[0]) || null,
          equipChosen: true, gripChosen: true, setupDone: false, tutExpanded: false, restExpanded: false, variationExpanded: false, variation: null,
          sets: [], note: '',
        });
      });
    });
  ...
  addSet() {
    const ex  = State.diary.workout[State.diary.activeIdx];
    const inp = State.diary.input || { type: 'work', weight: 0, reps: 1, rpe: null, mode: 'normal', stance: null, position: null, weightMode: null };
    const effectiveWeight = (inp.weightMode === 'bw') ? 0 : inp.weight;
    const newSet = { type: inp.type, weight: effectiveWeight, reps: inp.reps, rpe: inp.rpe, mode: inp.mode || 'normal', stance: inp.stance || null, position: inp.position || null, weightMode: inp.weightMode || null };
    ex.sets.push(newSet);
    addSet(ex.id, newSet); // DB layer: persist immediately
    const p  = prevOf(ex.id);
    const n  = ex.sets.length;
    const _isStance = STANCE_EX_IDS.has(ex.id);
    const ck = [ex.id, ex.equip, _isStance ? (ex.stance || 'shoulder') : null, !_isStance ? ex.gripType : null, !_isStance ? ex.gripWidth : null, ex.accent, ex.tut !== 'normal' ? ex.tut : null].filter(Boolean).join('_');
    ...
    if (!hasNonWork) {
      ...
      State.diary.input = { type:'work', weight: lastSet.weight, reps: lastSet.reps, rpe:null, mode: curMode, stance: curStance, position: curPosition, _hint: null };
    } else if (lastType === 'warmup') {
      // После разминки → подводящий +25%
      const nw  = r25(lastW * 1.25);
      const pct = Math.round((nw / lastW - 1) * 100);
      State.diary.input = { type:'leadup', weight: nw, reps: 5, rpe:null, mode: curMode, stance: curStance, position: curPosition, _hint: 'Подводящий: ' + nw + ' кг (+' + pct + '%)' };
    } else if (lastType === 'leadup') {
      if (lastW >= p.w * 0.82) {
        // Достаточно подводки → рабочий
        const ww = Math.max(r25(p.w), lastW);
        State.diary.input = { type:'work', weight: ww, reps: p.r, rpe:null, mode: curMode, stance: curStance, position: curPosition, _hint: null };
      } else {
        // Ещё один подводящий +25%
        const nw  = r25(lastW * 1.25);
        const pct = Math.round((nw / lastW - 1) * 100);
        State.diary.input = { type:'leadup', weight: nw, reps: 4, rpe:null, mode: curMode, stance: curStance, _hint: 'Подводящий: ' + nw + ' кг (+' + pct + '%)' };
      }
    } else {
  ...
  updateInput(field, val) {
    if (!State.diary.input) State.diary.input = { type: 'work', weight: 0, reps: 1, rpe: null, mode: 'normal' };
  ...
  updateExField(field, val) {
    ...
    if (field === 'gripType' || field === 'gripWidth') ex.gripChosen = true;
    if (field === 'stance') ex.stanceChosen = true;
    if (field === 'tutExpanded'         && !val) ex.tut       = 'normal';
    if (field === 'restExpanded'        && !val) ex.rest      = '120';
    if (field === 'variationExpanded'   && !val) ex.variation = null;
    App.render();
  },

  // Вариация: клик по активной → сбрасывает (null), по неактивной → выбирает
  setVariation(id) {
    const ex = State.diary.workout[State.diary.activeIdx];
    ex.variation = ex.variation === id ? null : id;
    App.render();
  },
  ...
  swapExercise(newId, muscle) {
    ...
    State.diary.workout[State.diary.activeIdx] = {
      ...
      gripType: 'straight', gripWidth: 'medium', stance: 'shoulder',
      equip: (newEx.equip && newEx.equip[0]) || 'barbell', tut: 'normal', rest: '120',
  ...
  startProgramWorkout(weekIdx, dayIdx, programId) {
    ...
    State.diary.plan.forEach(ex => {
      const exObj = { id: ex.exId, name: ex.name, muscle: ex.muscle, equip: ex.equip, accent: ex.accent, gripType: 'straight', gripWidth: 'medium', stance: 'shoulder', tut: 'normal', rest: '120', tutExpanded: false, restExpanded: false, variationExpanded: false, variation: null, equipChosen: true, gripChosen: true, setupDone: false };
      addExercise(exObj);
  ...
  addToPlan() {
    ...
    if (State.diary.midWorkoutAdd) {
      const block = {
        ...
        equip, accent, gripType: 'straight', gripWidth: 'medium', stance: 'shoulder', tut: 'normal', rest: '120',
        equipChosen: true, gripChosen: true, setupDone: false, tutExpanded: false, restExpanded: false, variationExpanded: false, variation: null,
        sets: [], note: '',
      };
    ...
    State.diary.plan.push({ exId, name: exData.name, muscle: sel.muscle, equip, accent, gripType: 'straight', gripWidth: 'medium', stance: 'shoulder' });
  ...
  startWorkout() {
    const blocks = State.diary.plan.map(p => ({
      ...
      equip: p.equip, gripType: p.gripType || 'straight', gripWidth: p.gripWidth || 'medium', stance: p.stance || 'shoulder', tut: 'normal', rest: '120',
      accent: p.accent,
      equipChosen: true, gripChosen: true, setupDone: false, tutExpanded: false, restExpanded: false, variationExpanded: false, variation: null,
      sets: [], note: '',
    }));
