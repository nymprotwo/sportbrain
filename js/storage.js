...
const Storage = {
  ...
  restore() {
    try {
      ...
      // Discard if no date (old format) or from a different day
      if (!data.savedDate) { this.clearCurrent(); return false; }
      const today = new Date().toDateString();
      if (new Date(data.savedDate).toDateString() !== today) {
        this.clearCurrent();
        return false;
      }
      ...
      // Don't force 'active' — browse screen shows resume/draft UI
      return true;
